...
from ...backend import pytorch as torch
import torch as T
...
class DeepONetCartesianProd(NN):
    ...
    def __init__(
        ...
        self.b = torch.Variable(torch.zeros(1, dtype=T.float32))
    ...
    def call(self, inputs, training=False):
        ...
        #x_loc = self.branch(x_loc)
        # Dot product
        if x_func.shape[-1] != x_loc.shape[-1]:
            raise AssertionError(
                "Output sizes of branch net and trunk net do not match."
            )
        if x_func.shape != T.Size([1]):
            x = T.einsum("bi,ni->bn", x_func, x_loc)
        else: 
            x = x_func*x_loc
        # Add bias
        x += self.b
...
class DeepONetDotProd(NN):
    """Deep operator network for dataset in the format of Dot product.
    ...
    def __init__(
        ...
        super(DeepONetDotProd, self).__init__()
        if isinstance(activation, dict):
            activation_branch = activation["branch"]
            self.activation_trunk = activations.get(activation["trunk"])
        else:
    ...
    def call(self, inputs, training=False):
        ...
        #x_loc = self.branch(x_loc)
        # Dot product
        if x_func.shape != x_loc.shape:
            raise AssertionError(
                "Output sizes of branch net and trunk net do not match."
            )
        x = T.sum(x_func*x_loc,-1).unsqueeze(1)
        # Add bias
        x += self.b
