...
def net_output_T(xy_data,t):
    ...
    return test_predict_HP.reshape(x.shape[0], y.shape[0])/K
...
def show_1D(fig,ax,data, x,text=0.0):
    ...
    ax.set_ylabel('heat flux [a.u.]', fontsize=16)
...
sigma = 1e-1
y_tile = np.linspace(0, width, Nwidth)
Uvec = create_pot(y_tile, A=(max_temp-min_temp), sigma=sigma*width, mu=width/2, T0=min_temp)
Tvec = Uvec/K
...
im1, im1_text, cbar1 = show_image(fig1, ax1, Unet*K, '0.1 s')
cbar1.ax.set_ylabel('Tile Potential [a.u.]',fontsize=16)
im1.set_clim(min_temp,max_temp)

fig2, ax2 = plt.subplots()
Umeas, HFmeas = Sample_theo(100)
im2, im2_text, cbar2 = show_image(fig2, ax2, Umeas*K, '0.1 s')
cbar2.ax.set_ylabel('Tile Temperature [a.u.]',fontsize=16)
im2.set_clim(min_temp,max_temp)

fig3, ax3 = plt.subplots()
im3, im3_text, cbar3 = show_image(fig3, ax3, np.abs(Umeas-Unet)*K, '0.1 s')
cbar3.ax.set_ylabel('Error Potential [a.u.]',fontsize=16)
im3.set_clim(0,0.005)
print(np.max(abs(HFmeas-HFnet))/np.max(HFmeas))
...
axrel.plot(y*width,np.ones_like(y)*2,'r--')
axrel.plot(y*width,np.ones_like(y)*-2,'r--')
axrel.set_ylabel('Heat flux [% of the maximum value]', fontsize=12)

fig4, ax4 = plt.subplots()
Umeas, _ = Sample_theo(1)
im4, im4_text = show_1D(fig4, ax4, Umeas[0,:]*K, y*width, 'Heat potential at the surface')
