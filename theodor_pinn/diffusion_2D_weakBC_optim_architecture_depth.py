...
from deepxde.deepxde.optimizers.config import set_LBFGS_options
...
set_LBFGS_options(iter_per_step=1000)
num_layers = [8,9]
for layers in num_layers:
    logname = 'theo_deepxde_BC_weak_alpha075_L-BFGS_32_layers_'+str(layers)
    ##Fix seed
    seed = 155
    ...
    early_stop = dde.callbacks.EarlyStopping(min_delta=1e-8, patience=1000)
    losshistory, train_state = model.train(epochs=20000, display_every=100, save_every=100,
                                           model_save_path='optimization/'+logname+'.pt',
                                           #model_restore_path=logname+'.pt',
                                           logname=logname,
                                           callbacks=[resampler, early_stop],
                                           Tensorboard=True, loss_names=loss_names)
    
    model.compile('adam', lr=3e-4, loss=loss, loss_weights=weights)
    resampler = dde.callbacks.PDEResidualResampler(period=1000)
    early_stop = dde.callbacks.EarlyStopping(min_delta=1e-8, patience=1000)
    losshistory, train_state = model.train(epochs=20000, display_every=100, save_every=100,
                                           model_save_path='optimization/'+logname+'.pt',
